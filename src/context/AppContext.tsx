...
const initialState: AppState = {
  ...
  engine: { state: 'idle', modelId: null, progress: 0, error: null, tokensPerSecond: 0, backend: null },
  generation: { temperature: 0.7, maxTokens: 512, contextLength: 2048, topP: 0.9, topK: 40, systemPrompt: '', nGpuLayers: 99999 },
  device: { ram: 4, storage: 0, hasWebGPU: false, tier: 'medium', isAutoDetected: true },
  sidebarOpen: true,
  downloads: [],
};
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const loadModelToEngine = useCallback(async (modelId: string) => {
    ...
    try {
      ...
      const backend = await engineRef.current.loadModelFromBlob(blob, state.generation.contextLength, state.generation.nGpuLayers);
      dispatch({ type: 'SET_ENGINE', engine: { state: 'ready', progress: 100, backend } });
      dispatch({ type: 'SET_ACTIVE_MODEL', id: modelId });
    } catch (err: any) {
      dispatch({ type: 'SET_ENGINE', engine: { state: 'error', error: err.message } });
    }
  }, [state.models, state.generation.contextLength, state.generation.nGpuLayers]);
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      await engineRef.current.generate(messages, {
        ...
        maxTokens: state.generation.maxTokens,
  ...
  return (
    <AppContext.Provider value={{
      ...
      loadModelToEngine, sendMessage, stopGeneration,
